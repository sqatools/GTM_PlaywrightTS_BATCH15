...
test.describe("handle multiple windows", () => {        
    ...
    test("scenario:1 one browser and two tabs", async ({ browser }) => {

        test.setTimeout(60_000)  // the test case will  //sets the maximum time allowed for the test case to run.
        ...
        await page1.locator('#fromcity').fill("hyderabad")

        await page1.locator('#destcity').fill('Mumbai')


        await page2.getByPlaceholder('Enter username').fill('User@gmail.com')
        await page2.getByPlaceholder('Enter password').fill('User@123')
     ...
     test("Scenario 3: One browser and| tab with click event ", async({browser}) => {
        test.setTimeout(60_000)
        const context: BrowserContext = await browser.newContext()
        ...
        const page2 = newPage
        await page2.getByPlaceholder("Email address or phone number").fill('User234@gmail.com')
        await page2.getByPlaceholder('Password').fill('123456')
        await page2.getByRole('button',{name :'Log in'}).click()

         await page1.getByPlaceholder("Enter username").fill("Playwright@gmail.com")
